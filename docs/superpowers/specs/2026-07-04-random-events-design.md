...
- **Target selection.** Region eligibility (typhoon/sky-lantern) = regions with **≥3 touching routes**
  ...
  sky-lantern takes **all** of them. Hotspot eligibility = cities with **≥2 incident routes**, excluding
...
| `events.VIRAL_HOTSPOT.name` / `.desc`                                          | 爆紅打卡站 / 該車站成為熱點：每次連接可 +1 分，建站更便宜                                                                                          | Viral Hotspot / A station goes viral: +1 per connection and cheaper stations                                                                               |
...
Note: the landed `events.VIRAL_HOTSPOT.desc` copy mentions "cheaper stations" — that phrasing predates
the final scoring-only mechanic (endpoint claim bonus, no station-cost discount) and is flavor text, not
a functional claim; flagged here rather than silently corrected since it's outside this doc's scope.
