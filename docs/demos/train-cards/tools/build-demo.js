...
<div class="wrap">
  ...
  <div class="notes">
    <dl>
      ...
      <dt>夜間塗裝</dt>
      <dd>
        原稿畫在近黑底上，車身因此接近純白。整片白直接放到深色卡面上會過亮——對卡面約 <b>12:1</b>，
        是深色介面裡最刺眼的東西。深色主題把每一階灰<b>依同一個比例壓向深色底</b>（<code>#1a1d20</code>，
        比純黑略冷一點），而不是換成另一組手挑的顏色：原稿的灰本來就是中性的，
        這裡多加任何色相都會變成「這台車被重新塗裝過」，而不是「這台車在暗處」。
        灰階之間的順序與間距都保留，所以每台車的立體感都在，只是不再刺眼——看起來仍然是同一台車。
      </dd>
      <dt>不動的部分</dt>
      <dd>
        塗裝色帶與 R20 的彩虹<b>完全不動</b>——那是卡片的身分，車身不再全白之後反而更跳。
        車窗與門框原本就是最深的一階，維持不變即可與壓暗後的車身拉開約 2:1，不需要改成「亮著的車窗」。
      </dd>
      <dt>唯一例外</dt>
      <dd>
        <b>30G1000型敞車</b>：整台車就是那組深藍黑，壓在最暗的卡面（約 <code>#24272a</code>）上只會變成一個洞，
        所以它反過來<b>提亮</b>成有光的鋼藍。車架留在共用的灰階上，仍高於提亮後的車身——敞車就是靠車架被認出來的。
      </dd>
      <dt>量測</dt>
      <dd>
        逐像素比對車輛與該張卡面的對比：九張卡「亮到刺眼（&gt;8:1）」的像素全部歸零，
        而決定車輛能不能被看成一個物件的<b>外輪廓</b>，低於 1.6:1 的比例也全部是 0%。
      </dd>
      <dt>切換方式</dt>
      <dd>
        夜間塗裝寫在插畫自己的 <code>&lt;style&gt;</code> 裡：以 <code>prefers-color-scheme</code> 為預設，
