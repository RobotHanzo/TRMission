// Route cartography + coastline smoothing — the PURE math behind the board's rendering,
// extracted from apps/web so the server's shared-map social cards draw with exactly the
// same curves, car slots, tunnel ties and hubs as the in-game map (they can never drift).
// The web's game/routeGeometry.ts re-exports everything here and adds its content-bound
// singletons on top; nothing in this module touches any content table.
 ...
 * "car-slot" segments (one rounded rectangle per train-length), the way the board reads.
 * Two problems the straight-line layout had, solved here geometrically:
 *
...
export function computeHubsFor(
  cities: readonly GeometryCity[],
  routes: readonly GeometryRoute[],
): Set<string> {
  ...
  for (const c of cities)
    if (!c.isIsland && (degree.get(c.id) ?? 0) >= HUB_MIN_DEGREE) hubs.add(c.id);
  return hubs;
}
 ...
 * route's on-board geometry — shared by the live map, the tutorial/encyclopedia specimens, and the
 * server's shared-map social card, so they are all produced by exactly the same math.
 */
function curveShape(
  a: { x: number; y: number },
  c: { x: number; y: number },
...
/**
 * Curved-route geometry (+ hub set) for an arbitrary city/route list — the single entry point
 * shared by the live board, the map builder's draft canvas, and the server's shared-map card.
 */
 ...
 * of the very same {@link curveShape} math as the live board.
 */
