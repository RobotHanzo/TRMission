...
const OUT = path.join(PKG, 'src', 'art', 'trainCars.ts');
...
//
// The nine rolling-stock illustrations, shared by apps/web (DOM SVG) and apps/mobile
// (react-native-svg). Each entry is one SVG body with every ink replaced by a \`$n\` placeholder
// into \`palette\`, so the night livery is a palette swap rather than a stylesheet — the same code
// path on both platforms. Resolve with \`trainCarArt(color, dark)\`.
//
// 610型 is the yellow car; 阿里山號 (the sheet's own yellow) is deliberately unused.
import type { CardColor } from '@trm/shared';

export interface TrainCarArtwork {
  /** Vehicle name as printed on the sheet. */
...
export const TRAIN_CAR_ART: Record<CardColor, TrainCarArtwork> = {
${built
  .map(
    (c) => `  ${c.key}: {
    name: ${q(c.name)},
...
export function trainCarArt(color: CardColor, dark: boolean): { viewBox: string; body: string } {
...
export function trainCarSvg(color: CardColor, dark: boolean): string {
  const { viewBox, body } = trainCarArt(color, dark);
