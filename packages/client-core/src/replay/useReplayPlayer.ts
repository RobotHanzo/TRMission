...
// Platform-free: the caller passes `reducedMotion` (each app's hook) via opts.
...
export interface ReplayPlayerOptions {
  /** Optional integrity seal — checked (and only warned about) at the final step. */
  finalDigest?: string | undefined;
  /** The platform's reduced-motion signal (shortens the tunnel-reveal autoplay hold). */
...
export function useReplayPlayer(
  ...
  opts: ReplayPlayerOptions = {},
): ReplayControls {
  const { finalDigest, reducedMotion = false } = opts;
