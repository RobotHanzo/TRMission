...
// limit + a 5/5s rate limit and rejects with these i18n keys). The web surfaces them as
// inline chat feedback instead of the generic action-rejected toast.
...
export const isChatRejectionKey = (key: string): boolean =>
  key === CHAT_TOO_LONG_KEY || key === CHAT_RATE_LIMITED_KEY;
