...
/** The Apple mark; inherits the button's text colour (white on the black SIWA button). */
const AppleIcon = () => (
  <svg
    className="oauth-icon"
    viewBox="0 0 24 24"
...
export function LoginScreen() {
  ...
  const modes: AuthMode[] = config
    ...
    : [];
  // Apple on the web rides the redirect flow, so it needs the Services ID (`appleRedirect`),
  // not just the native credential audiences (`apple`).
  const appleOn = !!config?.providers.appleRedirect;
  const hasOauth = !!config && (config.providers.google || config.providers.discord || appleOn);
        ...
        {hasOauth && (
          <>
            ...
            <div className="stack">
              ...
              {appleOn && (
                <a className="oauth-btn oauth-apple" href={oauthStartUrl('apple')}>
                  <AppleIcon />
                  {t('continueWithApple')}
                </a>
              )}
