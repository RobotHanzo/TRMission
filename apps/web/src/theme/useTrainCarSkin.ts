...
// mobile's hook of the same name cannot drift; only the store binding is per-platform.
...
/**
 * Fetch the offered packs once per session. Mounted from `App`, not from the card renderer —
 * a hand renders a dozen cards and each one must not issue a request. A failure leaves the list
 * null, which means "offer everything this build bundles"; skins are cosmetic, so a stale answer
 * costs nothing.
 */
