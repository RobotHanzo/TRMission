...
- **Terminate** (`games.terminate`): DB CAS `LIVE→TERMINATED` **first**, then `hub.evictMatch` (drains
  ...
  replayable.
- **Global config** (`config.features`, admin+): three singletons the admin app's Features panel edits —
  `PUT /dashboard/config/features` (default feature flags), `PUT /dashboard/config/official-maps`
  (which shipped maps players may pick; at least one must stay on, and a switch-off never touches a
  running game or its replay — `src/maps/CLAUDE.md`), and `PUT /dashboard/config/train-car-skins`
  (which train-card skin packs players may pick; the default pack is forced on and a switch-off is
  never retroactive to an account's stored choice — `src/skins/CLAUDE.md`). Audited as
  `config.features` / `config.officialMaps` / `config.trainCarSkins` with before/after.
