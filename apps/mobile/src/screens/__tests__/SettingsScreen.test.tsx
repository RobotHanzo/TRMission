...
describe('SettingsScreen index', () => {
  ...
  it('reflects a changed setting without a reload', async () => {
  ...
  it('each row pushes its own page', async () => {
    await renderIndex();
    await fireEvent.press(screen.getByTestId('settings-nav-appearance'));
    expect(nav.navigate).toHaveBeenCalledWith('SettingsAppearance');
    await fireEvent.press(screen.getByTestId('settings-nav-notifications'));
    expect(nav.navigate).toHaveBeenCalledWith('SettingsNotifications');
    await fireEvent.press(screen.getByTestId('settings-nav-account'));
    expect(nav.navigate).toHaveBeenCalledWith('SettingsAccount');
  });

  it('names the account, and calls a guest a guest', async () => {
    await renderIndex();
    expect(screen.getByText('Ada')).toBeTruthy();
    expect(screen.getByText('a@b.c')).toBeTruthy();

    useSession.setState({ user: { displayName: '訪客', isGuest: true } as never });
    await screen.rerender(<SettingsScreen navigation={nav as never} route={{} as never} />);
