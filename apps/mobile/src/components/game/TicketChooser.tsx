...
import { CountPill, TrayHead } from '../../theme/gameChrome';
...
export function TicketChooser({
  ...
  const chevronInk = tokens.inkSoft;
  ...
  return (
    <View {...anchor} style={styles.chooser} accessibilityLabel={t('chooseTickets')}>
      <TrayHead title={t('chooseTickets')} count={kept.size} />
      ...
      <Pressable
        style={[styles.peekToggle, { borderTopColor: tokens.line }]}
        accessibilityRole="button"
        accessibilityState={{ expanded: showHand }}
        onPress={() => setShowHand((v) => !v)}
      >
        {showHand ? (
          <ChevronDown size={15} color={chevronInk} />
        ) : (
          <ChevronRight size={15} color={chevronInk} />
        )}
        <Text style={[styles.peekLabel, { color: tokens.ink }]}>{t('cards')}</Text>
        <CountPill value={handCount} />
      </Pressable>
      ...
      >
        ...
        <Text style={[styles.peekLabel, { color: tokens.ink }]}>{t('tickets')}</Text>
        <CountPill value={keptTicketIds.length} />
      </Pressable>
